...
class RapidApplicationWithoutShutdownHook internal constructor(
    ...
    init {
        // Removed: Runtime.getRuntime().addShutdownHook(Thread(::shutdownHook))
    ...
    companion object {
        private val log = LoggerFactory.getLogger(RapidApplicationWithoutShutdownHook::class.java)

        fun create(env: Map<String, String>, configure: (ApplicationEngine, KafkaRapid) -> Unit = { _, _ -> }) = Builder(
            RapidApplicationConfig.fromEnv(env)
        ).build(configure, CIO)
    }
    ...
    class Builder(private val config: RapidApplicationConfig) {
        ...
        private val ktor = KtorBuilder()
            ...
            // Removed: .metrics(rapid.getMetrics())
        ...
        fun <TEngine : ApplicationEngine, TConfiguration : ApplicationEngine.Configuration> build(configure: (ApplicationEngine, KafkaRapid) -> Unit = { _, _ -> }, factory: ApplicationEngineFactory<TEngine, TConfiguration>): RapidsConnection {
            ...
            configure(app, rapid)
            return RapidApplicationWithoutShutdownHook(app, rapid, config.appName, config.instanceId, this::onKtorStartup, this::onKtorShutdown)
        }
